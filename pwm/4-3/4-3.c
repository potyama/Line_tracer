...
volatile char red[8] = "RED:";
volatile char green[9] = "GREEN:"; 
/* モータ制御関係 */
...
int main(void)
{
	...
	while (1){ /* 普段はこのループを実行している */
	  ...
	  /*if(redval >= 0 && redval < 10){ 
		red[4]= redval + '0';
		red[5] = '\0';
	}else if(redval >= 100){
		red[4] = (redval/100) + '0';
		red[5] = (redval/10)%10 + '0';
	      red[6] = (redval%10) + '0';
	      red[7] = '\0';
	}else{
	          red[4] = (redval/10) + '0';
		...
		green[6] = (greenval/100) + '0';
		green[7] = ((greenval/10)%10) +'0';
	      green[8] = (greenval%10) + '0';
	      green[9] = '\0';
	    }else{
	          green[6] = (greenval/10) + '0';
	          green[7] = (greenval%10) + '0';
	          green[8] = '\0';
	    }*/
...
=======
		if(dp < 0)dp = ADBUFSIZE -1;
>>>>>>> 706a4d0dd2964e1fb60db578d4d150d9390e1816
		ad += adbuf[ch][dp];
		dp--;
	}
	 ...
	 /* この関数はタイマ割り込み0の割り込みハンドラから呼び出される */
{
	...
	green[6] = (greenval/100) + '0';
	green[7] = ((greenval%100)/10) +'0';
	green[8] = (greenval%10) + '0';
	green[9] = '\0';
	//4-2
/*	if(key_read(2) == KEYPOSEDGE && redval < 255){
	redval++;
	}
	if(key_read(5) == KEYPOSEDGE && redval > 0){
	redval--;
	}
	if(key_read(3) == KEYPOSEDGE && greenval < 255){
	greenval++;
	}
	if(key_read(6) == KEYPOSEDGE && greenval > 0){
	greenval--;
	}
*/
	//4-3
