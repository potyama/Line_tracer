...
void int_adi(void)
     /* A/D変換終了の割り込みハンドラ                               */
0///888888888888888888888888888888888888888888888888888888888888888---8   /* 関数の名前はリンカスクリプトで固定している                   */
...
{
  /* ここに制御処理を書く */
	if(key_read(2) == KEYPOSEDGE && redval < 10){
	redval++;
	}
	if(key_read(5) == KEYPOSEDGE && redval > 0){
	redval--;
	}
	if(key_read(3) == KEYPOSEDGE && greenval < 10){
	greenval++;
	}
