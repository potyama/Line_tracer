		 ...
		 /*		  離されている，押されている，今押された，今離された，キーなし*/
{
	...
	if(r == KEYON && keyoldval[keynum] == KEYOFF){
		keyoldval[keynum] =keynewval[keynum] = KEYON;
		keyval = KEYPOSEDGE;
	}else if(r == KEYOFF && keyoldval[keynum] == KEYON){
		keyoldval[keynum] = keynewval[keynum] = KEYOFF;
		keyval = KEYNEGEDGE;
	}else{
		if(r == KEYTRANS){
